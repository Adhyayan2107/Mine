...
import { dailyLogs, journalEntries, habitCompletions, habits, todos, workoutSplitDays } from '../schema';
...
export async function getMonthActivity(
  ...
  const [logRows, journalRows, completionRows] = await Promise.all([
  ...
  function entryFor(date: string): DayActivity {
    ...
    if (!entry) {
      entry = { hasWeight: false, hasWorkout: false, habitsCompleted: 0, hasJournal: false };
      map.set(date, entry);
    }
    return entry;
  }
